...
//    protected HashMap<FDRLevel,HashMap<Integer,SubGroupFdrInfo>> GroupedFDRs = new HashMap<FDRLevel, HashMap<Integer, SubGroupFdrInfo>>();
     ...
     */
    public void setPeptides_directional(boolean peptides_directiona) {
        this.peptides_directional = peptides_directiona;
    }
...
// 
    public void writeFiles(String path, String baseName, String seperator, FDRResult result) throws FileNotFoundException {
        ...
        if (!csvSummaryOnly) {
            ...
            String header = getPSMHeader(seperator);
...
//            java.util.Collections.reverse(fdrPSM);
        ...
        for (PSM pp : psms) {
            ...
            String line = getPSMOutputLine(pp, seperator);
            if (!csvSummaryOnly)
                psmOut.println(line);
...
//            java.util.Collections.reverse(fdrPeptidePairs);
        ...
        for (PeptidePair pp : peps) {
            ...
            if (pp.isLinear() && ! pp.isLoop()) {
                ...
                if (!csvSummaryOnly) {
                    String line = getLinearPepeptideOutputLine(pp, seperator);
                ...
                if (!csvSummaryOnly) {
                    String line = getXlPepeptideOutputLine(pp, seperator);
...
//        CountOccurence<Integer> fdrPepPairLinearGroupCounts = new CountOccurence<Integer>();
//        java.util.Collections.sort(fdrLinearPeptidePairs);
////        if (!isPSMScoreHighBetter())
////            java.util.Collections.reverse(fdrLinearPeptidePairs);
//
//        int pepLinearT = 0;
//        int pepLinearD = 0;
//
//        if (!csvSummaryOnly) {
//            pepsOut = new PrintWriter(path + "/" + baseName + "_PeptidePairs_Linear" + extension);
//
//            pepsOut.println("ID" + seperator + "PSMIDs" + seperator + "Protein1" + seperator + "Decoy1" + seperator + "Protein2" + seperator + "Decoy2" + seperator + "Peptide1" + seperator + "Peptide2" + seperator + "fromSite" + seperator + "ToSite" + seperator + "psmID" + seperator + "Score" + seperator + "isDecoy" + seperator + "isTT" + seperator + "isTD" + seperator + "isDD" + seperator + "fdrGroup" + seperator + "fdr" + seperator + "" + seperator + "Protein1FDR" + seperator + "Protein2FDR" + seperator + "LinkFDR" + seperator + "PPIFDR");
//        }
//
//        for (PeptidePair pp : fdrLinearPeptidePairs) {
//            fdrPepPairLinearGroupCounts.add(pp.getFDRGroup());
//            String[] psmids = pp.getPSMids();
//
//            if (!csvSummaryOnly) {
//                pepsOut.println(pp.getPeptidePairID() + seperator + MyArrayUtils.toString(psmids, ";") + seperator + "\"" + pp.getPeptide1().getProteinGroup().acessions() + "\"" + seperator + pp.getPeptide1().isDecoy() + seperator + "\""
//                        + pp.getPeptide2().getProteinGroup().acessions() + "\"" + seperator + pp.getPeptide2().isDecoy() + seperator
//                        + "\"" + pp.getPeptide1().getSequence() + "\"" + seperator + "\"" + pp.getPeptide2().getSequence() + "\"" + seperator
//                        + pp.getPeptideLinkSite1() + seperator + pp.getPeptideLinkSite2() + seperator
//                        + pp.getTopPSMIDs() + seperator + pp.getScore() + seperator + pp.isDecoy() + seperator
//                        + pp.isTT() + seperator + pp.isTD() + seperator + pp.isDD() + seperator
//                        + pp.getFDRGroupName() + seperator + pp.getFDR()
//                        + seperator + seperator + (pp.getFdrProteinGroup1() == null ? "" : pp.getFdrProteinGroup1().getFDR()) + seperator + (pp.getFdrProteinGroup2() == null ? "" : pp.getFdrProteinGroup2().getFDR()));
//            }
//            if (pp.isTT()) {
//                pepLinearT++;
//            } else if (pp.isTD()) {
...
//            java.util.Collections.reverse(fdrProtainGroupLinks);
        ...
        if (!csvSummaryOnly) {
            ...
            String header = getLinkOutputHeader(seperator);
        ...
        for (ProteinGroupLink l : links) {
            ...
            if (!csvSummaryOnly) {
                ...
                String line  = getLinkOutputLine(l, seperator);
...
//            java.util.Collections.reverse(fdrProtainGroupPair);
        ...
        if (!csvSummaryOnly) {
            ...
            String header = getPPIOutputHeader(seperator);
        ...
        for (ProteinGroupPair pgp : ppis) {
            ...
            if (!csvSummaryOnly) {
                String line = getPPIOutputLine(pgp, seperator);
                ppiOut.println(line);
            }
...
//            java.util.Collections.reverse(fdrProteinGroups);
        ...
        for (ProteinGroup pg : pgs) {
            ...
            if (!csvSummaryOnly) {
                pgOut.println(pg.acessions() + seperator + pg.descriptions() + seperator + pg.getScore() + seperator + pg.isDecoy() + seperator
                        + pg.isTT() + seperator + pg.isTD() + seperator + pg.isDD() + seperator + pg.getFDRGroupName() + seperator + pg.getFDR());
            }
            if (pg.isDecoy()) {
                proteinGroupD++;
            } else {
...
//            java.util.Collections.reverse(fdrProteinGroups);
        ...
        summaryOut.println("\"peptide pair\"" + seperator + " " + target_pep_fdr);
        summaryOut.println("\"protein group\"" + seperator + " " + target_prot_fdr);
        summaryOut.println("Link" + seperator + " " + target_link_fdr);
        summaryOut.println("\"Protein Group Pair\"" + seperator + " " + target_ppi_fdr);
...
//            normDD = DD * DDfactor;
            ...
            if (efdr <= fdr ) {
                ...
                if ((efdr<prevFDR || prevFDR == Double.POSITIVE_INFINITY) || fdr == Double.POSITIVE_INFINITY) {
                    ...
                    if ((efdr_p / fdr < info.saftyfactor) || fdr == Double.POSITIVE_INFINITY)  {
...
//    
    ...
    protected ArrayList<String> getPSMOutputLine(PSM pp) {
        ...
        String[] line = new String[]{pp.getPsmID() 
                ...
                ,"\"" + accessions1+ "\"" 
                ,"\"" + descriptions1+ "\"" 
                , Boolean.toString(pep1.isDecoy()) 
                , "\"" + accessions2+ "\"" 
                ,"\"" + descriptions2 + "\"" 
                ,Boolean.toString(pep2.isDecoy()) 
                , "\"" + pepSeq1 + "\"" 
                ,"\"" + pepSeq2 + "\""
                , positons1 
                ,positons2 
