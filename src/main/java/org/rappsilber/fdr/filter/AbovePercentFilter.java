 ...
 */
public class AbovePercentFilter implements PSMFilter{
    /**
    ...
    public ArrayList<PSM> filter(ArrayList<PSM> psms) {
        ...
        for (Map.Entry<String,Object> e : infos.entrySet()) {
            ...
            if (o instanceof Comparable) {
                ...
                defineLimits(all, name, 0, lowLimit, 1, lowcutoffs, allFDR);

                defineLimits(all, name, all.size()-1, all.size()-lowLimit, -1, highcutoffs, allFDR);
        ...
        for (int i = start; i!=lowLimit; i+=increment) {
            ...
            if (window.size()>windowSize) {
                if (cutSensible == null) {
                    ...
                    cutSensible = cutSensibleFDR > allfdr;
                    if (!cutSensible) {
                        break;
                    }
                }
